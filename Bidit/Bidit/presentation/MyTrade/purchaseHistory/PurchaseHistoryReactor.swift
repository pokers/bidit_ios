...
extension PurchaseHistoryReactor {
    func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .viewDidLoad:
            ...
            return requestItemBuyList(userId: userId)
    ...
    //내 구매 리스트 요청
    func requestItemBuyList(userId : Int) -> Observable<Mutation>{
        LoadingIndicator.showLoading()
        return Observable<Mutation>.create(){ emitter in

            
            Network.shared.apollo.fetch(
                query: GetMyBiddingQuery(biddingQuery: .init(status: nil,
                                                             itemId: nil,
                                                             price: nil)),
                ...
                case .success(let data) :
                    ...
                    do {
                        ...
                        data.data!.getMyBidding?.forEach{item in
                            var node = item?.item
